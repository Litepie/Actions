 ...
 */
trait HandlesNotificationsAndEvents
{
    /**
    ...
    /**
     * Fire Laravel events after action completion
     */
    protected function fireEvents(): void
    {
        ...
        foreach ($events as $eventClass) {
            ...
            try {
                event(new $eventClass($this->result, $this->data, $this->user));
            } catch (\Throwable $e) {
                Log::error("Failed to fire event: {$eventClass}", [
                    'error' => $e->getMessage(),
                    'action' => static::class
                ]);
            }
    ...
    /**
     * Get events to fire after action completion
     */
